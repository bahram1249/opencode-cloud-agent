...
Per-workspace credential storage and container environment injection. Each workspace stores its own provider API keys and GitHub tokens, isolated from other workspaces. Supports interactive GitHub OAuth scoped to a workspace and a guided setup wizard.
