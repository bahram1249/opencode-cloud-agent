...
Automatic project synchronization: cloning missing repositories on project add, syncing all enabled projects on session creation or container ensure, and supporting explicit sync requests.

## Requirements

### Requirement: Auto-clone on project add

When a project is added with a `remoteUrl`, the system SHALL clone it immediately if the target path does not exist in the container.
...
- **WHEN** a user requests project sync for a workspace
