...
Session output telegrams stream live independently of which session is "active" or which workspace the user is viewing. Users can switch between sessions without losing visibility into any running session's output, and switch-back creates a fresh live view while preserving previous ones.
