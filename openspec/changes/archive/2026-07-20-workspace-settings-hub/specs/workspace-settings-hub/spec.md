...
### Requirement: Model configuration per workspace only

The system SHALL store the default model at the workspace level only. Per-project provider and model configuration SHALL be removed. The Workspace model's `model` field SHALL be the single source of truth for which model the workspace uses.

#### Scenario: Set default model via Settings
- **WHEN** user selects a model from the paginated model picker on the Settings screen
- **THEN** the system SHALL update `Workspace.model` for the workspace
- **AND** the selected model SHALL be used for all sessions in that workspace

#### Scenario: Per-project model commands removed
- **WHEN** user runs `/project provider`, `/project model`, or `/project models`
- **THEN** the system SHALL respond with a message that model configuration is now at workspace level and direct the user to the Settings screen
